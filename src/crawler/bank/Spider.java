...
public class Spider {
	...
	public static List<String[]> captureCardType(String url, String textName){
		//connect Jsoup
...
public static boolean searchMatrix(int[][] matrix, int target) {
    ...
    return recursive(matrix,0,matrix[0].length-1,target);
}
public static boolean recursive(int[][] matrix, int row, int column, int target) {
    if(row>=matrix.length||column<0) return false;
    int value = matrix[row][column];
    if(value<target) return recursive(matrix,++row,column,target);
    if(value>target) return recursive(matrix,row,--column,target);
    return true;
}
...
//	    Spider.captureCardType(url,textName);
		int[][] i = {{-5}};
		int target = -2;
		System.out.println(searchMatrix(i,target));
